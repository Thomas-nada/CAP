...
export function renderNav(state) {
    const isLoggedIn = !!state.ghToken;

    // "New CAP" only visible to logged-in users
    const menu = [
    ...
    const sessionControls = isLoggedIn ? `
        <button onclick="window.logout()" class="flex items-center gap-3 pl-2 pr-4 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all group">
            ...
            <div class="hidden lg:block text-left">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">${state.ghUser?.login || ''}</p>
                <p class="text-xs font-bold text-red-500 group-hover:text-red-600">Logout</p>
            </div>
        </button>
    ` : `
        <button onclick="window.loginWithGitHub()" class="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl text-sm font-bold transition-all shadow-lg">
